...
async function weatherCheck(city, name) {
  if (locationInput.value === '' || nameInput.value === '')
    return;
  ...
  // Update the display with the user's name
  document.querySelector('.weather-details h2').textContent = `Weather for ${name}`;
...
searchBtn.addEventListener('click', () => {
  weatherCheck(locationInput.value, nameInput.value);
});